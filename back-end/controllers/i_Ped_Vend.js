...
const I_ped_vend = require('../models/I_ped_vend')
...
controller.novo = async (req, res) => {
    ...
    try {
        await I_ped_vend.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch(erro) {
...
controller.listar = async (req, res) => {
    try {
        let dados = await I_ped_vend.find() // Traz todos os itens cadastrados
...
controller.obterUm = async (req, res) => {
    try {
        ...
        let obj = await I_ped_vend.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        let ret = await I_ped_vend.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        ...
        let ret = await I_ped_vend.findByIdAndDelete(id)
